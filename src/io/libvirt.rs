...
use crate::core::verification_model::{LibvirtGuestHandle, VerificationScenarioDefinition};
use std::fs;
use std::path::Path;
...
impl LibvirtCommandRunner {
    ...
    fn install_ignition(
        ...
        let (butane_path, local_ignition_path) =
            self.render_ignition(workspace_root, domain_name, network)?;
...
impl VerificationLibvirtBoundary for LibvirtCommandRunner {
    fn create_guest(
        ...
        let (ignition_path, local_butane_path, local_ignition_path) =
            self.install_ignition(workspace_root, &domain_name, &network)?;
        ...
        let guest_ip = if self.network_mode == "static" {
            network
                .allocated_ip
                .clone()
                .ok_or_else(|| CoreError::new(FailureClass::Apply, "static network mode missing allocated IP"))?
        } else {
            self.wait_for_guest_ip(&domain_name)?
        };
        ...
        Ok(LibvirtGuestHandle {
            ...
            ssh_target: format!("{}@{}", self.ssh_user, guest_ip),
            connection_uri: self.connection_uri.clone(),
            workspace_root: guest_root.display().to_string(),
            env_backed: true,
            network_mode: Some(self.network_mode.clone()),
