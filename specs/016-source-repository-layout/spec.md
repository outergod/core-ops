...
- **FR-002**: System MUST treat each service's `config/` directory as deploying to `/etc/<config-root>/`, where `<config-root>` is taken from `services/<svc-id>/service.yaml` if that file exists, otherwise defaults to `<svc-id>`.
