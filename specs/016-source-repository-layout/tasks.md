...
- [X] T104 [P] [US1] Author `specs/016-source-repository-layout/examples/04-host-overlay/` — base service with units, plus a host directly under `hosts/<id>/<svc>/` contributing a drop-in and a `config/` whole-file replacement; `README.md`
...
- [ ] T406 **(release-blocker)** VM-backed scenario at `tests/fixtures/verification/scenarios/source-repo-variant-config-root.yaml` — DEFERRED. The script needs a libvirt-managed VM via the existing `core-ops-verify run` harness, which is not available in this dev environment. CI's `e2e-gate.yml` is the binding gate; running locally requires the operator's libvirt host. Exempted-with-justification: the unit and integration test suite covers the variant config-root path comprehensively (`test_source_repo_layout::example_02_variant_config_root_loads`, `test_service_selection`, plus T302's migration assertion); the VM run validates only that the planned `/etc/<config-root>/...` paths actually appear on the live host filesystem. The release tag SHOULD NOT be cut until T406 passes against a libvirt host.
