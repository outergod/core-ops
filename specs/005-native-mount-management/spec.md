...
- Q: What bounded path preparation is allowed for mount targets? -> A: CoreOps may create the mount target directory identified by `Where=` before activation, but it must not become a general directory management system.
- Q: Should services depend on mounts by path only or by an explicit stable CoreOps identifier? -> A: Services depend on native managed mount references derived from the `.mount` unit stem, not on a separate stable CoreOps identifier.
...
- A declared mount target directory does not exist yet and must be prepared before mount activation.
...
- **FR-003**: The system MUST allow a selected service definition to declare one or more required managed mount references, where each reference is the native `.mount` unit stem derived from the corresponding managed mount artifact's target path, and those required mounts MUST be available before the service is treated as successfully runnable.
- **FR-003a**: Service definitions are the authoritative source of service-to-mount relationships. Embedded `[X-CoreOps]` metadata on managed mount artifacts MUST NOT define, infer, or override consumer relationships, and MUST NOT introduce a second logical mount identifier separate from the native unit naming.
...
- **FR-005**: The system MUST restrict the embedded `[X-CoreOps]` metadata to CoreOps-specific reconciliation semantics that native systemd sections do not already express cleanly, including bounded mountpoint-creation policy for the mount unit's `Where=` path.
...
- **FR-007**: The system MUST validate managed mount artifacts before execution, including mount target path validity, duplicate or conflicting definitions, required dependency references, consistency between the native unit contents and the embedded `[X-CoreOps]` metadata, native-unit compatibility rules, and correct alignment between the `.mount` unit stem, the mount target path, and any optional `.automount` companion.
...
- **FR-013c**: Host overrides for service-referenced managed mounts MUST NOT change the effective native `Mount` `Where=` path, because service mount references are derived from the native `.mount` unit stem generated from that path.
- **FR-014**: The system MUST support removing a previously managed mount reference and reconcile the host back to the declared state without leaving ambiguous ownership of the mount or automount units. When removal is required, the system MUST stop dependent managed services first, remove generated mount or automount units only after the mount is no longer active, and fail explicitly if the mount remains busy or cannot be cleanly deactivated or removed.
...
- **FR-016**: The system MUST reject or clearly fail reconciliation when a dependent service references a managed mount stem that is not declared by any selected service-owned native mount artifact.
- **FR-017**: The system MUST preserve explicit operator diagnostics when mount reconciliation fails, including which managed mount stem failed, why verification failed, and which dependent services were blocked or degraded as a result.
...
- **Managed Mount Artifact**: A user-authored native `.mount` artifact, plus an optional `.automount` companion that shares the same systemd path-derived stem, carrying bounded `[X-CoreOps]` reconciliation metadata.
- **CoreOps Mount Metadata**: The bounded `[X-CoreOps]` metadata embedded in a managed mount artifact for reconciliation-only semantics not expressed by native systemd sections.
- **Mount Dependency**: A service-declared requirement on one or more managed mount references derived from native `.mount` unit stems, materialized through native path-based and explicit unit dependencies.
...
### `[X-CoreOps]` Metadata Schema

`[X-CoreOps]` metadata is intentionally narrow. It augments native `.mount` and `.automount` units with reconciliation-only metadata that systemd does not interpret itself.

Allowed fields on managed `.mount` artifacts:

- **`CreateMountpoint`**: Optional boolean, default `true`. When `true`, CoreOps may create the mount target directory identified by `Where=` before activation if it does not already exist. When `false`, reconciliation fails instead of creating the mountpoint.

Allowed fields on managed `.automount` artifacts:

- No `[X-CoreOps]` fields are required on `.automount` artifacts in this iteration.
- If an `[X-CoreOps]` section is present on an `.automount` artifact, it MUST NOT introduce additional semantics beyond what the matching `.mount` artifact already declares.

Disallowed `[X-CoreOps]` fields in this iteration:

- No separate stable identifier field such as `Id`
- No duplicate target-path field such as `PreparedPath`
- No consumer or service-relationship fields
- No ownership or mode fields such as `PreparedOwner`, `PreparedGroup`, `PreparedMode`, or `PreparedServiceConsumed`
- No policy enums such as `RemovalPolicy` or `VerificationMode`

Field behavior and validation rules:

- `CreateMountpoint` applies to the mount unit's native `Where=` path; it does not define a second path.
- `[X-CoreOps]` fields follow native unit layering semantics, so later effective values override earlier ones after layering.
- CoreOps validates the effective merged `[X-CoreOps]` section after layering and rejects unsupported fields or unsupported values.
- `.automount` and `.mount` artifacts remain coupled by their shared native path-derived stem rather than by a separate CoreOps identifier.
...
- **Declarative state model**: Desired state includes managed native mount artifacts plus embedded `[X-CoreOps]` metadata and service-to-mount dependencies by native mount reference; observed state captures native mount/unit presence and verification status; plans describe mount, preparation, and service actions; outcomes report success, failure, and blocked dependencies.
...
- Bounded path preparation is limited to optionally creating the mountpoint identified by `Where=` and any necessary parent directories.
...
- Managed mount references are derived from native `.mount` unit stems; services depend on those references rather than on raw paths alone or a separate `[X-CoreOps]` identifier.
