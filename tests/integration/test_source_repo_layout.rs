...
struct HostGuard(Option<OsString>);
...
fn git_init_commit(repo: &Path) -> String {
    Command::new("git")
        .arg("init")
        .arg(repo)
        .output()
...
fn materialize_example(name: &str) -> (TempDir, String) {
...
fn materialize_skeleton() -> (TempDir, PathBuf, PathBuf) {
    let tmp = TempDir::new().expect("tempdir");
...
fn write_host_yaml(hosts_dir: &Path, host: &str, services: &[&str]) {
...
fn load_with_host(repo: &Path, rev: &str, host: &str) -> Result<DesiredState, RepoError> {
    let _lock = path_lock().lock().expect("path lock");
    let _guard = HostGuard(std::env::var_os(HOST_OVERRIDE_ENV));
