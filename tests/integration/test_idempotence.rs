...
#[test]
fn repeated_runs_remain_converged() {
    ...
    let repo = temp_dir("core_ops_repo_idempotence");
    let rev = init_git_repo(&repo);

    let temp = temp_dir("core_ops_idempotence");
    ...
    let second = reconcile_apply(&deps).expect("apply again");
