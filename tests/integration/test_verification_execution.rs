...
#[derive(Default)]
struct RebootDisconnectGuestBoundary;

impl VerificationGuestBoundary for RebootDisconnectGuestBoundary {
    fn wait_ready(
        &self,
        guest: &core_ops::core::verification_model::LibvirtGuestHandle,
        timeout: &str,
    ...
    fn run_command(
        ...
        if command.trim() == "sudo systemctl reboot" {
            Ok(core_ops::core::verification_model::GuestCommandOutput {
                status_code: 255,
                stdout: "connection closed for reboot".to_string(),
                stderr: String::new(),
            })
        } else {
            Ok(core_ops::core::verification_model::GuestCommandOutput {
...
#[test]
fn reboot_step_tolerates_expected_disconnect_exit_code() {
    let _env_guard = core_ops_bin_env_lock().lock().expect("env lock");
    let mut scenario = load_scenario_definition(&fixture_path(
        "tests/fixtures/verification/scenarios/minimal-accepted.yaml",
    ))
    .expect("scenario");
    scenario.scenario_id = "reboot-disconnect".to_string();
    scenario.title = "Reboot disconnect remains recoverable".to_string();
    scenario.description = "A reboot SSH disconnect should not fail the scenario by itself.".to_string();
    scenario.steps = vec![
        VerificationScenarioStep {
            step_id: "boot".to_string(),
            step_type: VerificationStepType::Boot,
    ...
    let guest = RebootDisconnectGuestBoundary;
    let collector = ArtifactCollector;
    let context = VerificationExecutionContext {
        workspace: workspace.path(),
        artifacts_root: artifacts.path(),
    ...
    let reboot_step = view
        ...
        .expect("reboot step");
    assert_eq!(reboot_step.status, core_ops::core::types::VerificationStepStatus::Passed);
    assert_eq!(reboot_step.exit_code, Some(255));
}

#[derive(Default)]
struct RebootFailureGuestBoundary;

impl VerificationGuestBoundary for RebootFailureGuestBoundary {
    fn wait_ready(
        &self,
        guest: &core_ops::core::verification_model::LibvirtGuestHandle,
        timeout: &str,
    ...
    fn run_command(
        ...
        if command.trim() == "sudo systemctl reboot" {
            Ok(core_ops::core::verification_model::GuestCommandOutput {
                status_code: 1,
                stdout: "reboot command failed".to_string(),
                stderr: String::new(),
            })
        } else {
            Ok(core_ops::core::verification_model::GuestCommandOutput {
...
#[test]
fn reboot_step_fails_on_unexpected_non_zero_exit_code() {
    let _env_guard = core_ops_bin_env_lock().lock().expect("env lock");
    let mut scenario = load_scenario_definition(&fixture_path(
        "tests/fixtures/verification/scenarios/minimal-accepted.yaml",
    ))
    .expect("scenario");
    scenario.scenario_id = "reboot-failure".to_string();
    scenario.title = "Reboot failure is not masked".to_string();
    scenario.description = "Unexpected reboot command failure must fail the scenario.".to_string();
    scenario.steps = vec![
        VerificationScenarioStep {
            step_id: "boot".to_string(),
            step_type: VerificationStepType::Boot,
    ...
    let guest = RebootFailureGuestBoundary;
    let collector = ArtifactCollector;
    let context = VerificationExecutionContext {
        workspace: workspace.path(),
        artifacts_root: artifacts.path(),
    ...
    assert_eq!(reboot_step.exit_code, Some(1));
}

#[derive(Default)]
struct RebootTransportFailureGuestBoundary;

impl VerificationGuestBoundary for RebootTransportFailureGuestBoundary {
    fn wait_ready(
        &self,
        guest: &core_ops::core::verification_model::LibvirtGuestHandle,
        timeout: &str,
...
#[test]
fn reboot_step_does_not_treat_generic_ssh_255_as_success() {
    let _env_guard = core_ops_bin_env_lock().lock().expect("env lock");
    let mut scenario = load_scenario_definition(&fixture_path(
        "tests/fixtures/verification/scenarios/minimal-accepted.yaml",
    ))
    .expect("scenario");
    scenario.scenario_id = "reboot-transport-failure".to_string();
    scenario.title = "Generic SSH 255 is not treated as reboot success".to_string();
    scenario.description =
        "A reboot step must not pass when ssh exits 255 for an unrelated transport/auth failure."
            .to_string();
    scenario.steps = vec![
        VerificationScenarioStep {
            step_id: "boot".to_string(),
    ...
    let guest = RebootTransportFailureGuestBoundary;
    let collector = ArtifactCollector;
    let context = VerificationExecutionContext {
        workspace: workspace.path(),
        artifacts_root: artifacts.path(),
    ...
    assert_eq!(reboot_step.status, core_ops::core::types::VerificationStepStatus::Failed);
    assert_eq!(reboot_step.exit_code, Some(255));
    assert_eq!(reboot_step.stderr.as_deref(), Some("Permission denied (publickey)."));
}
...
impl VerificationGuestBoundary for RebootTimeoutGuestBoundary {
    ...
    fn run_command(
        ...
        if command.trim() == "sudo systemctl reboot" {
            Err(CoreError::new(
                FailureClass::Transient,
                "ssh reboot command timed out after 1s",
            ))
        } else {
            Ok(core_ops::core::verification_model::GuestCommandOutput {
                status_code: 0,
...
#[test]
fn reboot_step_classifies_transient_command_errors_as_timeout() {
    let _env_guard = core_ops_bin_env_lock().lock().expect("env lock");
    let mut scenario = load_scenario_definition(&fixture_path(
        "tests/fixtures/verification/scenarios/minimal-accepted.yaml",
    ))
    .expect("scenario");
    scenario.scenario_id = "reboot-timeout".to_string();
    scenario.title = "Reboot timeout is classified explicitly".to_string();
    scenario.description = "Transient reboot command timeout should classify the scenario as timeout.".to_string();
    scenario.steps = vec![
        VerificationScenarioStep {
            step_id: "boot".to_string(),
            step_type: VerificationStepType::Boot,
    ...
    let guest = RebootTimeoutGuestBoundary;
    let collector = ArtifactCollector;
    let context = VerificationExecutionContext {
        workspace: workspace.path(),
        artifacts_root: artifacts.path(),
    ...
    let reboot_step = view
        ...
        .find(|step| step.step_id == "reboot")
        .expect("reboot step");
    assert_eq!(reboot_step.status, core_ops::core::types::VerificationStepStatus::TimedOut);
    assert!(reboot_step
        .details
        .as_deref()
        .unwrap_or("")
