...
fn verification_rules_accept_volume_inactive() {
    ...
    let temp = temp_dir("core_ops_verify_rules");
    ...
    let volume_ok = result.verification_results.iter().any(|res| {
        res.target == "gamma.volume"
            && res.status == core_ops::core::types::VerificationStatus::Success
    });
    assert!(volume_ok);
}
