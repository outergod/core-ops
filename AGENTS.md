...
- Rust (stable toolchain, edition 2021) + clap, thiserror, miette, log, systemd-journal-logger, tempfile, serde; likely add `serde_json` for the canonical status snapshot (004-reconcile-provenance)
...
## Recent Changes
- 004-reconcile-provenance: Added Rust (stable toolchain, edition 2021) + clap, thiserror, miette, log, systemd-journal-logger, tempfile, serde; likely add `serde_json` for the canonical status snapshot
